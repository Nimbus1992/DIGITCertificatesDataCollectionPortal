...
export function ComplaintsRoadmapEditor() {
  const { data, set } = useStore();
  const rm: CmsRoadmapData = data.cmsRoadmap;
  const [tab, setTab] = useState<Tab>('releases');

  function upd(patch: Partial<CmsRoadmapData>) {
    set('cmsRoadmap', { ...rm, ...patch });
  }

  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-6">
      <div>
        <h2 className="text-xl md:text-2xl font-bold text-gray-900 mb-1">CMS Roadmap Editor</h2>
        <p className="text-gray-500 text-sm">Complaint Management System — edit all roadmap sections</p>
      </div>
